...
# 고객 고유 ID 별로 주문 날짜를 정렬하여, 각 고객의 주문 횟수 및 날짜 차이 계산
low_aov_customers_orders = orders_customers_merged[orders_customers_merged["Customer_unique_id"].isin(aov_group.index)].sort_values(by=["Customer_unique_id", "Order_purchase_timestamp"])

# 고객별 주문 횟수 계산
customer_reorder_counts = low_aov_customers_orders.groupby("Customer_unique_id").size()
...
# 재구매한 고객들의 첫 구매와 두 번째 구매 날짜 차이 계산 및 출력
for customer_unique_id in reordered_customers:
    customer_orders = low_aov_customers_orders[low_aov_customers_orders["Customer_unique_id"] == customer_unique_id]
    if customer_orders.shape[0] >= 2:  # 재구매한 고객인 경우에만 계산
        first_order_date = pd.to_datetime(customer_orders.iloc[0]["Order_purchase_timestamp"])
        second_order_date = pd.to_datetime(customer_orders.iloc[1]["Order_purchase_timestamp"])
        # 첫 번째 구매와 두 번째 구매 사이의 시간 차이 계산
        date_diff = second_order_date - first_order_date
        days = date_diff.days
        hours, remainder = divmod(date_diff.seconds, 3600)
        minutes, seconds = divmod(remainder, 60)
        print(f"고객 고유 ID: {customer_unique_id}, 첫 구매와 두 번째 구매 사이의 간격: {days}일, {hours}시간, {minutes}분")
